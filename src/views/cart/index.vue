    ...
    <div class="container">
      ...
      <div class="cart">
        <table>
          <thead>
            <tr>
              <th width="120"><XtxCheckbox :modelValue="$store.getters['cart/isAllSelected']" @change="checkAll($event)">全选</XtxCheckbox></th>
      ...
      <div class="action">
        <div class="batch">
          <XtxCheckbox @change="checkAll($event)">全选</XtxCheckbox>
...
export default {
  ...
  setup () {
    ...
    const batchDeleteCart = (isTrue) => {
      proxy.$confirm({ text: `是否确认${isTrue ? '清空失效商品' : '批量删除'}` }).then(() => {
    ...
    const checkout = () => {
      if (store.getters['cart/selectedList'].length === 0) return proxy.$message({ text: '至少选择一件商品' })
      // 如果登录直接跳转
      if (store.state.user.profile.token) {
        return router.push('/member/checkout')
      }
      proxy.$confirm({ text: '下单结算需要登录，确认现在去登录吗？' }).then(() => {
